/* ============================================================
   ReportDocument — THÂN bản báo cáo Spy Ads (bản in / PDF).
   Nguồn DUY NHẤT của bố cục báo cáo, dùng cho cả hai nơi:
     1. WeeklyReportModal  — xem trước + nút "Tải PDF về" trong trình duyệt
     2. src/report-ssr.tsx — kết xuất phía máy chủ để gửi email (Puppeteer)
   Sửa bố cục ở đây thì cả hai nơi đổi theo.
   Component THUẦN trình bày: không hook, không chạm window/document
   -> render được bằng renderToStaticMarkup trong Node.
   ============================================================ */
import React from "react";
import type { SpyDashboardData, DataSourceType, SpyReport } from "../types";
import { viLabel, isAdsDisclaimer } from "../utils/spyData";
import { buildReportModel, type ReportBrandRow, type ReportModel } from "../utils/reportData";
...
/** Dựng props cho ReportDocument từ dataset thô (dùng chung web + máy chủ). */
export function buildReportDocumentProps(
  data: SpyDashboardData,
  dataSource: DataSourceType,
): { m: ReportModel; report: SpyReport | null } {
  return { m: buildReportModel(data, dataSource), report: latestWeeklyReport(data) };
}

export interface ReportDocumentProps {
  m: ReportModel;
  report: SpyReport | null;
  /** Logo: "/seryn-mark.png" trên web, data-URI khi render phía máy chủ. */
  logoSrc?: string;
}

export default function ReportDocument({ m, report, logoSrc = "/seryn-mark.png" }: ReportDocumentProps) {
  ...
  <article id="seryn-report-doc" className="rpt-doc">
    ...
    <header className="rpt-header">
      <div className="rpt-header-top">
        <img src={logoSrc} alt="SERYN" className="rpt-logo" />
    ...
    <footer className="rpt-footer">
      <div className="rpt-footer-brand">SERYN Clinic · Insight Guard</div>
