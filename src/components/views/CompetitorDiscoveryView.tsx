    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white border border-dashed border-slate-300 rounded-xl p-10 text-center">
          ...
          <p className="text-sm font-bold text-slate-700">Run Competitor Discovery Manual workflow to find new competitors from Exa.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-7 gap-3">
        <Stat label="Found" value={counts.found} />
        <Stat label="Ready for spy" value={counts.ready} accent />
        <Stat label="Needs page_id" value={counts.needs_page_id} />
      ...
      {latestRun && <p className="text-xs text-slate-500">Latest run: <b>{latestRun.discovery_run_id}</b> ({latestRun.status}) @ {latestRun.finished_at || latestRun.started_at}</p>}

      {/* C. Filters */}
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative">
          <Search className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-2.5" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm brand / domain / service..."
          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-50 text-slate-400 font-mono uppercase tracking-wider">
              <tr className="text-left">
                <th className="py-2.5 px-3">Brand</th><th className="px-2">Links</th><th className="px-2">page_id</th>
                <th className="px-2">Services</th><th className="px-2">Conf.</th><th className="px-2">Status</th><th className="px-2">Spy</th><th className="px-2 text-right">Action</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c) => (
              ...
              {!filtered.length && <tr><td colSpan={8} className="py-4 text-center text-slate-400">Không có candidate khớp bộ lọc.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800 space-y-1">
        <p className="flex items-center gap-1.5 font-bold"><AlertTriangle className="w-3.5 h-3.5" /> Lưu ý import</p>
        <p>• Candidate không có numeric <code>page_id</code> sẽ ở trạng thái <b>needs_page_id</b> — ScrapeCreators chưa thể spy ads tin cậy.</p>
        <p>• Vanity Facebook URL (facebook.com/brand) KHÔNG phải page_id. Đừng bịa page_id.</p>
        <p>• Sau khi <b>approve</b> (page_id numeric, confidence ≥ 0.65), chạy <code className="font-mono">npm run competitors:import</code> để đưa vào tab <b>Competitors</b>. Lần weekly spy tiếp theo ScrapeCreators sẽ crawl.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <tr className="border-t border-slate-100 hover:bg-slate-50/60 align-top">
      <td className="py-2.5 px-3">
        ...
        {c.duplicate_of && <p className="text-[10px] text-slate-400">dup of {c.duplicate_of}</p>}
      ...
      <td className="px-2">
        {editing ? (
          <div className="flex items-center gap-1">
            <input value={pid} onChange={(e) => setPid(e.target.value)} className="w-24 border border-slate-200 rounded px-1 py-0.5 text-[11px]" placeholder="numeric id" />
      ...
      <td className="px-2">
        <div className="flex items-center justify-end gap-1">
          <button title="Approve" className="p-1 rounded hover:bg-emerald-50 text-emerald-600" onClick={() => onAct(() => approveCandidate(c))}><Check className="w-3.5 h-3.5" /></button>
          <button title="Reject" className="p-1 rounded hover:bg-rose-50 text-rose-500" onClick={() => onAct(() => rejectCandidate(c))}><X className="w-3.5 h-3.5" /></button>
          <button title="Mark duplicate" className="p-1 rounded hover:bg-slate-100 text-slate-500" onClick={() => onAct(() => markDuplicate(c))}><Copy className="w-3.5 h-3.5" /></button>
        </div>
      </td>
    </tr>
  );
...
function Header({ writeOn }: { writeOn: boolean }) {
  return (
    <div>
      <h2 className="text-xl font-extrabold text-slate-900">Competitor Discovery</h2>
      <p className="text-sm text-slate-500">
        Đối thủ mới do Exa phát hiện — review &amp; approve trước khi import vào Competitors.
        {writeOn ? " Thao tác sync lên Google Sheets." : " Chưa cấu hình write — thao tác lưu draft local."}
      </p>
    </div>
  );
}
